...
bool PongMode::update(float elapsed) {

	static std::mt19937 mt; //mersenne twister pseudo-random number generator

	if (left_score == 12 * 19 || right_score == 12 * 19) {
		if (left_score == 12 * 19) {
	...
	{ //right player ai:
		...
		if (ball.x <= left_paddle.x) {
			if (right_paddle.y < left_paddle.y + ai_offset) {
				right_paddle.y = std::min(left_paddle.y + ai_offset, right_paddle.y + 2.0f * elapsed);
			} else {
				right_paddle.y = std::max(left_paddle.y + ai_offset, right_paddle.y - 2.0f * elapsed);
			}
		} else {
	...
	if (ball.x < 0) {
		speed_multiplier = 3.0f * std::pow(2.0f, left_score / 80.0f);
	} else {
		speed_multiplier = 3.0f * std::pow(2.0f, right_score / 80.0f);
	}
	...
	//----- grass update -----
	for (float i = (ball.x - 0.2); i < (ball.x + 0.3); i += 0.2) {
		for (float j = (ball.y - 0.2); j < (ball.y + 0.3); j += 0.2) {
			if (i >= -6.5 && j >= - 4.5 && i < 7 && j < 5){
				uint32_t tmp_x = uint32_t(i * 2 + 13);
				uint32_t tmp_y = uint32_t(j * 2 + 9);
				if (grass[tmp_x][tmp_y] == 0) {
					grass[tmp_x][tmp_y] = 1;
					if (tmp_x < 12) {
						left_score += 1;
					}
...
void PongMode::draw(glm::uvec2 const &drawable_size) {
	...
	const glm::u8vec4 grass_color = HEX_TO_U8VEC4(0x2eff94ff);
	const glm::u8vec4 water_color = HEX_TO_U8VEC4(0x00f4e7ff);
	...
	for (uint32_t i = 0; i < 12; i++) {
		for (uint32_t j = 0; j < 19; j++) {
			if (grass[i][j] == 0) {
				draw_rectangle(glm::vec2((float(i) - 13)/2.0, (float(j) - 9)/2.0), glm::vec2(0.25f, 0.25f), grass_color);
			}
		}
	}

	//Right Grass
	for (uint32_t i = 15; i < 27; i++) {
		for (uint32_t j = 0; j < 19; j++) {
			if (grass[i][j] == 0) {
				draw_rectangle(glm::vec2((float(i) - 13)/2.0, (float(j) - 9)/2.0), glm::vec2(0.25f, 0.25f), grass_color);
			}
	...
	//River
	draw_rectangle(glm::vec2(0.0, 0.0), glm::vec2(0.75, 4.75), water_color);
	...
	for (uint32_t i = 0; i < int(left_score / 11.4); ++i) {
	...
	for (uint32_t i = 0; i < int(right_score / 11.4); ++i) {
		draw_rectangle(glm::vec2( court_radius.x - (2.0f + 3.0f * i) * score_radius.x, court_radius.y + 2.0f * wall_radius + 2.0f * score_radius.y), score_radius, fg_color);
	}
